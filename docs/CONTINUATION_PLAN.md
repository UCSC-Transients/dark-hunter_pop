...
| Q17 (2026-09-24) | **Multi-solution `source_id`s are real and must be reproduced, not collapsed — in both the real data and the mock.** Gaia's NSS pipeline sometimes publishes more than one row for a `source_id` (distinct from the 6-group cross-match fan-out of #221/PR #240): (1) cross-type multiplicity — different `nss_solution_type` families for the same source (e.g. `Orbital` + `SB1`); (2) same-type multiplicity — more than one `Orbital` solution at different (aliased) periods for the same source. Ryan Foley's ruling, verbatim: "This means, I think, that we should treat the simulations in the same way. Some should have these different solutions, and thus the simulations should match each sample, including duplicates," and, confirming this covers case 2 as well, "Yes, the multiple Orbital solutions should also be reproduced. Go ahead with the plan." Binding consequences: `data_acquisition.py` tags multi-solution rows by `nss_solution_type` and keeps them (never merges/collapses); `forward_model.py`'s `selection_function_astrometric` and `selection_function_followup` must both reproduce the empirical multi-solution emission rate (cross-type and same-type/period-aliasing) so a mock system can emit more than one row at the real rate; `population_model.py`/`inference.py` must count a multi-row system consistently on both sides of the real-vs-mock comparison so multiplicity is never double-weighted. See `docs/ARCHITECTURE.md` §4 "Multi-solution sources" for the full write-up. Issues #237 (domain question), #231 (data_acquisition duplicate handling). |
